    ...
    GuiFocusService guiFocusService) : ISceneService
{
    ...
    private Result OnElementAdded(GuiElementArgs args) => AddState(args.NodeId, GuiNodeState.Show | GuiNodeState.Enabled);
    private Result OnElementRemoved(GuiElementArgs args) => RemoveState(args.NodeId, GuiNodeState.All);
    ...
    private Result AddState(Id<GuiNode> nodeId, GuiNodeState guiNodeState)
        => Result.Try<Exception>(() => stateService.UpdateState(nodeId, s => s | guiNodeState));
