...
- [ ] Result-handling policy & ORES001 adoption epic (Technical):
  - Description: Olve.Results 0.48 ships an analyzer (ORES001) flagging every unobserved `Result`. A full-solution build surfaces ~198 sites. Also exposes a live crash: "cannot afford track" is returned as a `ResultProblem` and propagated into the fatal frame-loop boundary, so the game *shuts down* when a player can't afford a track (`TrackPlacingService.cs:21` → `TrackPlacingToolService.cs:134` → `GameManager.Update` `Stop()`). Full design: `scratchpad/result-handling-design.md`. **Model (settled): every fault is non-fatal to the process; handled as low as possible; the top is a defect alarm, not a crash.** Layers: (domain) recover + CLEANUP + log-once, else propagate with `Prepend`; (runner: Scene/SceneManager) contain any child fault — returned problem OR caught exception — log-once + retry next frame, rethrow exceptions in Debug; (top: GameManager) a fault that escaped containment is UNACCEPTABLE — Prod: log-critical (greppable marker + full context) + write a crash report (serialized `ResultProblem` + log snapshot, to `crash-reports/`) + **return to main menu**; Debug: rethrow. No new problem types, no severity bit, no quarantine (all deferred as "smarter later" — see Olve.Utilities#73). Because there's no quarantine/hard-stop, **G2 cleanup is the load-bearing anti-corruption safeguard** — recovery must fix/remove the faulty entity so it stops failing. Migration runs ≥10 sites per batch, by submodule; tree stays green via a temporary `WarningsNotAsErrors;ORES001` exemption removed in the final step.
  ...
  - [ ] Step 3: runner isolation — `Scene.Update/Render/Input` + `SceneManager.Update/Input/Render`: wrap each child in try/catch, contain (returned problem OR exception) → log-once → continue (retry next frame), rethrow exceptions in Debug, preserve `Input` `Pass.Block`. Then `GameManager`: replace `Stop()` with log-critical + crash-report + return-to-main-menu (needs a NEW "abandon session → main menu" path). Fault-injection test: sibling still runs, fault logged, frame survives.
