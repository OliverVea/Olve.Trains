...
- [ ] Result-handling policy & ORES001 adoption epic (Technical):
  ...
  - [ ] Step 2: crash fix (independent, tool-path only) — handle "cannot afford" at the call site in `TrackPlacingToolService` (log at info — normal player outcome — and move on; never propagate). Verify running: drain the player's balance via the real placement flow → placing track does NOT shut the game down. (Command path is NOT fatal — `CommandRunner` returns to CLI, not `GameManager` — so no command-handler fix needed.)
