...
- [ ] Architecture & quality guardrails epic (Tooling/Testing):
  - Description: Turn the Roslyn code metrics (`docs/architecture-diagram/code-metrics.json` + `CODE_METRICS.md`, surfaced as overlays in the interactive architecture diagram) and the architectural rules already documented in CLAUDE.md into *automated* guardrails, so changes (human or agent) can't silently erode cohesion or cross architectural boundaries. Cohesion/architecture are enforced as boundary fitness functions; per-file quality as a no-regression ratchet. The gate is a single agent-runnable script (`scripts/quality.sh`) the agent runs before raising a CR — **not** wired into the CD pipeline.
  - [x] **Canonical gate** `scripts/quality.sh` — single agent-runnable check; the agent runs it green before raising a CR. **Local only — NOT wired into the CD pipeline** (deliberately; the pipeline is untouched).
  - [x] Architecture fitness tests — `tests/Olve.Architecture.Tests/` (NetArchTest.eNhancedEdition; original is unmaintained), **blocking**. Encodes: engine ∌ game; `Scenes/GameLogic` ∌ `GameRendering`/`GameUI`. (Dropped a "GameLogic ∌ Commands" rule — command handlers legitimately co-locate with their domain; kept it from being a false-positive generator.) Extend with more boundary rules as needed.
  - [x] Code-metrics tool in-repo + portable — `tools/code-metrics/` (Roslyn `CodeAnalysisMetricData`, resolves the bundled engine via the NuGet package path); `tools/code-metrics/baseline.json` committed as the baseline. Refresh with `bash scripts/metrics-refresh.sh`.
  - [x] Metrics ratchet — `scripts/metrics-ratchet.py` flags touched files losing >5 MI or gaining coupling, and new files below the MI/complexity bar. **Advisory** for now (set `QUALITY_STRICT=1` to block); promote to blocking after a calibration period.
  - [x] CLAUDE.md invariants — added a "Quality Gate (IMPORTANT!)" section: run `scripts/quality.sh` before a CR, what it enforces, how to refresh the baseline.
  - [ ] After a calibration period, consider promoting the ratchet to blocking and adding more fitness rules (e.g. data/record types stay behavior-free, tier-direction). (Pipeline wiring intentionally out of scope.)
  - [x] Spike: do teams successfully wire metric-threshold breaches into an *agent* that extracts meaningful action? (findings below)
  - Spike findings (research, 2026-06):
    - **Gates:** change-scoped *no-regression* gates + structural fitness functions hold the line; absolute thresholds get gamed/disabled. **Do not gate on Maintainability Index** (LOC-dominated, averaged, un-actionable); cyclomatic complexity is largely a LOC proxy — use complexity/coupling as *triage*, not gate targets. Prioritise by **churn × complexity** (hotspots). Baseline existing violations (ArchUnit `freeze()` / `NetArchTest.eNhancedEdition` — the original NetArchTest is unmaintained). No hard independent proof gates improve long-term quality, but the usability case for diff-scoped ratchets is solid.
    - **Agent-on-breach:** real but *narrow*. **CodeScene ACE is the only off-the-shelf tool that triggers on a metric regression AND validates the fix against the metric** (refactors the 5 complexity-class smells). Generic AI-fix tools (Copilot Autofix, Sonar AI CodeFix, Amazon Q, the PR-review bots) trigger on security/lint/diff, *not* metric breaches. Raw LLM refactoring alters behaviour ~6–7% of the time → needs a deterministic safety net (regression tests must pass + re-measure the metric; the "RefactoringMirror" pattern). Independent field data (GitClear) shows AI correlates with *more* duplication / *less* refactoring, and reviewer fatigue is the dominant complaint.
    - **Decision:** build the gate first (fitness tests hard-fail; complexity/coupling ratchet advisory). Defer the agent loop; if added, make it **narrow**: trigger only on a *worsened* metric in a churn hotspot scoped to the diff → agent proposes a PR showing before/after metric delta → human approves → must pass the full test suite *and* a re-measured improvement → never auto-merge. Instrument acceptance/revert rates and kill it if it produces churn not cohesion. Evaluate CodeScene ACE as the turnkey option vs. a home-grown Roslyn-metric → agent → re-measure loop.
