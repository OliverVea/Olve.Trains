...
- [ ] Result-handling policy & ORES001 adoption epic (Technical):
  - Description: Olve.Results 0.48 ships an analyzer (ORES001) flagging every unobserved `Result`. A full-solution build surfaces ~198 sites. Also exposes a live crash: "cannot afford track" is returned as a `ResultProblem` and propagated into the fatal frame-loop boundary, so the game *shuts down* when a player can't afford a track (`TrackPlacingService.cs:21` → `TrackPlacingToolService.cs:134` → `GameManager.Update` `Stop()`). **Model: problems are non-fatal unless critical.** A problem is critical when its `Severity >= ProblemSeverities.Critical` (existing field, checked with `problems.AnyCritical()`). Layers: (domain) handle as low as possible — recover + CLEANUP + log, else propagate with `Prepend`; (runner: `Scene`) log a service's non-critical problem once per failure streak (`FaultLogger`) and keep running its siblings, propagate critical ones; (top: `GameManager`) a critical problem is logged at critical level and stops the game. Exceptions are not caught. Because non-critical faults just log and repeat, **G2 cleanup is the load-bearing anti-corruption safeguard** — recovery must fix/remove the faulty entity so it stops failing. Migration runs ≥10 sites per batch, by submodule; tree stays green via a temporary `WarningsNotAsErrors;ORES001` exemption removed in the final step.
  - [x] Step 1: bump Olve.* family to 0.48 (requires .NET SDK 10.0.3xx for the newer Roslyn the analyzer needs), regenerate Meshes/Textures catalogs, add the temporary ORES001 warning exemption to keep the tree green/runnable
  - [x] Step 2: crash fix (independent, tool-path only) — handle "cannot afford" at the call site in `TrackPlacingToolService` (log at info — normal player outcome — and move on; never propagate). Verified running via the real tool flow (`scripts/replays/verify-cant-afford-crashfix.py`): draining the balance then placing a track through the tool no longer shuts the game down; the reverted-fix negative control reproduces the fatal `Stop()` chain. (Command path is NOT fatal — `CommandRunner` returns to CLI, not `GameManager` — so no command-handler fix needed.)
  - [x] Step 3: runner containment — `Scene.Input/Update/Render` log a service's non-critical problem once per failure streak (`FaultLogger`) and continue with the remaining services (a faulting `Input` counts as `Pass.Pass`; `Pass.Block` preserved); only critical problems propagate. `GameManager` stops only on critical problems (logged at critical level) and logs non-critical frame update/render problems once per failure streak. No try/catch. Tests: `SceneFaultContainmentTests` (sibling still runs, fault logged once, recovery logged, critical propagates, input block preserved).
  - [ ] Step 4: batched ORES001 cleanup — B3 tools first, then GameUI/GUI, GameRendering, Shared/GUI, GameLogic (entity loops get recover+CLEANUP+log-once), long tail; then test projects (Layout, Slider/Checkbox/RadioButton/Animation). ≥10 sites per batch. G2 cleanup is the load-bearing per-site judgment (not mechanical) — a bare `continue` leaving a zombie is forbidden.
  - [ ] Step 5: remove the ORES001 exemption from `Directory.Build.props` — full build green with ORES001 fatal again. Update CLAUDE.md + olve-results skill with the layered failure-handling model.
  - Separate bug (not blocking, own task): `PlaceTrack` charges (`TryCharge`) before placing; if a sub-track fails after, money is gone with no refund (a G2 violation). Fix with proper refund/rollback semantics (single-segment = full refund; multi-subtrack = decide atomic-rollback vs partial).
- [ ] GUI click dispatch epic (Technical):
  - Description: Every GUI click goes through one global `GuiActivationService.GuiElementActivated` event, so each panel subscribes, receives every click in the game, and works out whether it was its own button through a private `NodeIdMatches` helper (8 copies in 7 files; `GuiElementService.IsElementNodeId` already does the same and is used only by `MainMenuService`). Element IDs are shared by every mounted copy of a layout, so each check also needs the registration ID. Move the check into the engine: handlers are registered per (registration, element), dispatched by node ID, and removed with their registration. Do this before the result-handling epic's GameUI/GUI batch, so that batch doesn't fix code this deletes. Later ideas, not part of this epic: a mounted-layout handle that owns anchor + registration + handlers, text/list bindings instead of per-frame polling, a shared world-selection service for "click with no tool active", and one parameterized building-placement tool instead of five copies.
  - [ ] Step 1: engine — `OnActivated(registrationId, element, handler)` on `GuiElementService` (or a small dedicated service): resolve the node when the handler is registered, keep a node → handler map, one subscription to `GuiElementActivated`, handlers removed by `UnregisterElementAndChildren`; keep the global event for `activate-gui`/`query-gui` and logging. Unit tests
  - [ ] Step 2: migrate the panel services (Station/Industry info panels, Depot panel, Signal rules panel, BurgerMenu, InfoBar, ToolBar, MainMenu) to it; delete every `NodeIdMatches` and manual subscribe/unsubscribe (fixes `InfoBarService` never unsubscribing)
  - [ ] Step 3: route widget value-changed events (slider, checkbox, radio button, dropdown) by node + registration instead of element ID, so two mounted copies of a layout don't collide
  - [ ] Step 4: update the `gui` skill (click handling, panel template, `StationInfoPanelService` example)
- [x] SceneManager refactor epic (Technical):
  - Description: `SceneManager` tracked each loaded scene across five hand-synced dictionaries (`_loadedScenes`, `_children`, `_scopes`, `_scopeOwner`, `_sceneParameters`), which caused a disposed-scope leak in `SceneScopeAccessor`, half-finished unloads, and a duplicated load path. Each loaded scene is now one `LoadedScene` in an `EntityStore` keyed by scene ID; parents, children and scope ownership are derived from it. Scene parameters are typed (`SceneKey<T>` + `SceneArguments`), and off-thread scene preparation is being removed.
  - [x] Step 1: `LoadedScene` store + ordered cache, typed scene parameters, `SceneScopeAccessor` read from `SceneManager`, best-effort tree unload, Olve.* 0.50.0
  - [x] Step 2: lineage-aware activation (activate pulls in ancestors, deactivate pushes to descendants), `LoadAndActivateScene` rollback on failure, `UnloadScene` replaces `DeactivateAndUnloadScene`, private load/activate/deactivate primitives; engine builds and tests pass
  - [x] Step 3: remove off-thread scene preparation (`PrepareScene`, `CommitPreparedScene`, `PreparedScene`, `LoadingService` background task); update the scenes skill and `docs/architecture.md`
  - [x] Step 4: constructor-injected scene parameters — apply arguments before resolving a scene's services, register a scoped holder per `SceneKey<T>` with a defaults factory (loading without arguments uses the defaults), inject the arguments (or per-concern slices) into service constructors; remove `ISceneParameterService`, `GameSceneParameterService`, `LoadingSceneParameterService` and the services' `*Override` setters
