...
- [ ] SceneManager refactor epic (Technical):
  ...
  - [ ] Step 1: `LoadedScene` store + ordered cache, typed scene parameters, `SceneScopeAccessor` read from `SceneManager`, best-effort tree unload, Olve.* 0.50.0
  - [ ] Step 2: lineage-aware activation (activate pulls in ancestors, deactivate pushes to descendants), `LoadAndActivateScene` rollback on failure, `UnloadScene` replaces `DeactivateAndUnloadScene`, private load/activate/deactivate primitives; engine builds and tests pass
  - [ ] Step 3: remove off-thread scene preparation (`PrepareScene`, `CommitPreparedScene`, `PreparedScene`, `LoadingService` background task); update the scenes skill and `docs/architecture.md`
