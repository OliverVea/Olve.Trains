...
- [ ] Result-handling policy & ORES001 adoption epic (Technical):
  - Description: Olve.Results 0.48 ships an analyzer (ORES001) that flags every unobserved `Result`. A full-solution build surfaces ~198 sites. Two entangled problems: (1) under-observation (dropped results — what ORES catches); (2) over-propagation — e.g. "cannot afford track" is returned as a `ResultProblem` and propagated into the fatal frame-loop boundary (`GameManager.Update`), so the game *shuts down* when the player can't afford a track (`TrackPlacingService.cs:21` → `TrackPlacingToolService.cs:134` → `Stop()`). Policy (locked): tag problems by interface — `IValidationProblem` (player-caused domain rejection: skip + feedback, never crash) vs `ISystemProblem` (engine failure). Validation is handled at the call site; unhandled system problems hit a top-level backstop: crash in Debug (fail loud), log+continue in Release (game survives). Backstop is an injectable policy service (testable), not `#if DEBUG`. Classification uses interface-tagged `ResultProblem` subclasses on plain `Result`/`Result<T>` (source generator NOT adopted now — see Olve.Utilities#73 for the enhancement that would make it adoptable later). Migration runs ≥10 sites per batch, by submodule, keeping the tree green via a temporary `WarningsNotAsErrors;ORES001` exemption removed in the final step.
  ...
  - [ ] Step 2: add `ISystemProblem`/`IValidationProblem` + `SystemProblem`/`ValidationProblem` subclasses; change the frame-loop backstop to debug-fatal / release-log via an injectable policy service; unit-test both paths
  - [ ] Step 3: crash fix — make "cannot afford" a `ValidationProblem` in `TrackPlacingService` + `BuildingService`; handle at call site (tools AND command handlers) as skip + log, don't propagate; verify running (place track while broke → no crash)
  - [ ] Step 4: batched ORES001 cleanup — B3 tools first, then GameUI/GUI, GameRendering, Shared/GUI, GameLogic, long tail; then test projects (Layout, Slider/Checkbox/RadioButton/Animation). ≥10 sites per batch, apply the call-site rule per site.
  - [ ] Step 5: remove the ORES001 exemption from `Directory.Build.props` — full build green with ORES001 fatal again. Update CLAUDE.md + olve-results skill with the taxonomy + backstop policy.
